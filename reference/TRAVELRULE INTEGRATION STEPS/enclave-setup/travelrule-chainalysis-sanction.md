...
송신 VASP와 수신 VASP 모두 Sanction API를 통한 사용자 계정의 추가 리스크 평가를 진행하여 규제 요건으로 활용할 수 있습니다. 단, Sanction API 호출 전 반드시 사용자 검증 API(POST /verifications)가 먼저 호출되어야 합니다. 구현 가이드는 다음과 같습니다.
