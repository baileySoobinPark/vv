...
hidden: false
---
This API allows the beneficiary VASP to query the ordering VASP for the status of a transaction. It is typically used when the beneficiary VASP has completed user verification but has not received a Transaction Report or Error Report from the ordering VASP within a certain timeframe.
...
2. If no verification history is found, the beneficiary VASP waits for a period of time for the ordering VASP to call the Transaction Report API.
