...
최종 검증 결과를 `verificationResult`필드에 반환해야 합니다. 검증 결과 해당 전송건에 문제가 없다고 판단하는 경우 `VERIFIED`로 설정하고, 수신자 정보를 IVMS101으로 함께 반환합니다. 정보를 검증할 수 없거나 검증 결과에 문제가 있는 경우 검증 결과를 `DENIED`로 설정하고, 아래 실패 사유 코드 중 하나를 `reason`에 함께 반환해야 합니다.
