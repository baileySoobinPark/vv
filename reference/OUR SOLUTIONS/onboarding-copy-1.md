...
title: Onboarding
excerpt: >-
  본격적인 Travel Rule 및 VerifyName 연동에 앞서, VerifyVASP 회원 가입을 완료하고 API Key를 발급받는 절차를
  진행해야 합니다.
deprecated: false
hidden: false
metadata:
...
아래 절차에 따라 VerifyVASP 콘솔을 통해 회원가입을 진행하면, Travel Rule 또는 VerifyName 프로토콜 연동을 위한 VerifyVASP 온보딩을 완료할 수 있습니다.
...
VerifyVASP 온보딩을 위해 필수 기업 정보를 모두 입력해야 합니다. 추가 서류가 필요한 경우, VerifyVASP 운영팀 담당자가 ‘Business Contact’ 탭에 입력된 이메일 주소로 개별 연락을 드릴 수 있으므로 반드시 수신 가능한 이메일을 정확히 기재해 주시기 바랍니다.
...
회원가입 요청이 완료되면 VerifyVASP 운영팀에서 가입 승인 절차를 진행합니다. 이 과정에서 추가 서류가 요청될 수 있으며, 모든 자료의 검토가 완료되면 최종적으로 가입이 승인됩니다.

보다 자세한 절차는 아래 첨부된 안내 영상을 참고해주세요.
...
> 3. **API Key를 사용하기 위해서는 Access Key와 Secret Key를 Enclave 서버의 환경 변수로 설정해야 합니다.**\
