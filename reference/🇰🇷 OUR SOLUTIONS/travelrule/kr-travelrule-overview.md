...
* **공개 키 전달 (Ordering VASP → Beneficiary VASP)**
  ...
  * 수신 VASP는 요청 데이터 서명에 포함된 송신 VASP의 공개키를 사용하여 서명을 검증하고 서명이 유효한 경우 해당 키를 캐싱 정책에 따라 저장합니다.
...
1. **주요 시나리오 및 흐름 검토**
   ...
   구현에 앞서 [Best Practice](ref:travelrule-flow-diagram) 를 통해 입출금 시나리오에서의 요청 순서와 통신 흐름을 확인합니다.
2. **구현 범위 확인 및 계획 수립**
...
7. **Robot VASP를 통한 TraveRule 시나리오 테스트**
