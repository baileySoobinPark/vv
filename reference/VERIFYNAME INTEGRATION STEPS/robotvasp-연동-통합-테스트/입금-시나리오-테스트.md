...
입금 시나리오 테스트는 규제 준수 의무가 있는 VASP와의 가상 자산 송수신 상황을 가정한 사전 검증 입금 테스트, 규제 준수 의무가 없는 VASP에게 가상 자산을 수신한 상황을 가정한 사후 검증 입금 테스트가 있습니다. 각 시나리오에 대한 테스트를 진행해 귀사가 구현한 VerifyName 프로토콜이 정상적으로 동작하는지 확인하십시오.
