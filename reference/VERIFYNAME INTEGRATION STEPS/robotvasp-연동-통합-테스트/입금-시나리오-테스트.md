...
입금 시나리오는 귀사 VASP의 규제 준수 여부에 따라 사전 검증, 사후 검증 두 가지 케이스로 구분합니다. 귀사의 VASP가 규제를 준수하여 자산을 전송하기 전 수신자를 확인해야하는 의무가 있는 VASP라면 사전 검증 출금 시나리오를, 규제를 준수할 의무가 없는 VASP라면 사후 검증 출금 시나리오를 진행하십시오.
