...
* **조건**
  ...
  * Owner Verification의 타입은 `VerifyBeneficiary` 이어야 합니다.
  ...
  <Accordion title="Owner Verification Simulation API 호출 방법" icon="fa-info-circle">
    **Method**: `POST`
...
* **조건**
  * Robot VASP가 Owner Verification Test를 진행해 반환받은 결과를 바탕으로 판단한 최종 검증 결과를 보고합니다. Report Verification Result Simulation API를 호출해 Robot VASP가 최종 검증 결과를 보고하도록 명령합니다.
