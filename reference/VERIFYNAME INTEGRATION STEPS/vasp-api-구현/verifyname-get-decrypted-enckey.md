...
* 환경 변수를 통해 직접적으로 키를 주입하고자 하는 경우,`VEGA_ENCRYPTION_KEY_BASE64`에 키 값을 설정합니다. Enclave는 해당 값을 그대로 데이터 암복호화에 사용합니다.
...
* `VEGA_ENCRYPTION_KEY_BASE64`환경 변수에 대칭 키를 평문으로 저장하지 마십시오. 이는 보안 상 매우 위험하므로 평문 키 대신 참조값만 저장하는 방식을 사용하는 것을 권장합니다.
